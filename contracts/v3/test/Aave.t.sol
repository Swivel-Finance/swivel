...
contract AaveTest is Test {
    ...
    address aaveAddr = 0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9;
    ...
    function setUp() public {
        ...
        swivel = new Swivel(address(marketplace), aaveAddr);

        // Approve Aave to spend from swivel
        vm.startPrank(address(swivel));
        IERC20(Constants.USDC).approve(aUSDC, type(uint256).max);
        IERC20(Constants.USDC).approve(aaveAddr, type(uint256).max);
        vm.stopPrank();
    ...
    function aaveDeal(address r, uint256 a) internal {
        ...
        vm.startPrank(r);
        IAave(aaveAddr).deposit(Constants.USDC, a, r, 0);
        vm.stopPrank();
    }
}
